...
const timeUnit = (step: number, name: string): TimeUnit => ({ step, name });
...
const padDate = (value: number | string, num: number): string => {
...
export const formatDateTime = (datetime: Date | number): string => {
...
export const formatDuration = (millis: number): string => {
  ...
  let nextUnitIndex = 1;
  while (nextUnitIndex < TIME_UNITS.length && remaining / TIME_UNITS[nextUnitIndex].step >= TIME_UNIT_THRESHOLD) {
...
export const debugGenerator = (namespace: string): Debug.IDebugger => Debug(`playwright-cluster: ${namespace}`);
